...
const CollectionPage = ({ collectionData }) => {
  ...
  if (collectionData["response"] === "Server Error") {
    return (
      <>
        <Navbar />
        <h2>Something went wrong with the server. Please try again.</h2>
        <Footer />
      </>
    );
  }
  ...
  const [anyNFTavailable, setanyNFTavailable] = useState(true);
  ...
  const saveCollectionContract = async (contract) => {
    await fetch(
      "http://localhost:8000/nft/updateCollection?collectionid=" +
        collectionData["collectionURL"] +
        "&contract=" +
        contract
    );
    getTokenFromContract(contract);
  };

  const saveNFTdata = async (deployed_contract_address, tokenId, tokenURI) => {
    let formData = new FormData();
    let updateURL = "http://localhost:8000/nft/updateNFT";
    formData.append("owner", account);
    formData.append("metadataURL", tokenURI);
    formData.append("collection", collectionData["collectionURL"]);
    formData.append("on_sale", "False");
    formData.append("is_minted", "True");
    formData.append("price", collectionData["collectionPrice"]);
    formData.append("contract_address", deployed_contract_address);
    ...
    await fetch(updateURL, {
      ...
      .then((resp) => {
        router.push("/assets/" + deployed_contract_address + "_" + tokenId);
        return resp.json();
      })
      .then((data) => {
        return data;
  ...
  const deployContract = async () => {
    if (collectionData["collectionContract"] === "0x") {
      setLoadingStatus("Deploying Contract...");
      ...
      await mycontract
        ...
        .send({ from: account }, (err, transactionHash) => {
          setContractStatus("Contract creation: " + transactionHash);
        })
        .on("confirmation", () => {
        ...
        .then((newContract) => {
          setContractStatus(
            "Contract deployed at: " + newContract.options.address
          );
          saveCollectionContract(newContract.options.address);
          setCollectionContract(newContract.options.address);
        });
    } else {
      getTokenFromContract(collectionData["collectionContract"]);
    }
  };

  const getTokenFromContract = async (deployed_contract_address) => {
    const deployed_contract = new Contract(
      nft_contract["abi"],
    ...
    await deployed_contract.methods
      ...
      .call()
      .then((res) => {
        createCollectible(deployed_contract_address, parseInt(res));
      });
  };
  const createCollectible = async (
    ...
    let tokenIdArray = [];
    for (let i = 0; i < collectionData["collectionSupply"]; i++) {
      if (!metadataArray[i]["is_minted"]) {
        remainingNFTArray.push(metadataArray[i]);
        tokenIdArray.push(i);
      }
    }
    let tokenId =
      tokenIdArray[
        Math.floor(Math.random() * 10000) % remainingNFTArray.length
      ];
    if (
      remainingNFTArray.length === parseInt(collectionData["collectionSupply"])
    ) {
      tokenId = getRarest();
    }
    let tokenURItoMint = JSON.parse(collectionData["collectionMetadata"])[
      "nft" + tokenId
    ];
    await deployed_contract.methods
      .createCollectible(tokenURItoMint)
      .send({ from: account })
      .on("transactionHash", (hash) => {
        setMintStatus("Minting Transaction: " + hash);
      })
      .on("confirmation", () => {
      ...
      .on("error", (err) => {
        ...
        setLoadingStatus("Minting Failed.");
  ...
  const sendFloorPrice = async () => {
    ...
    const toWeit = 10 ^ 18;
    let value =
      parseFloat(collectionData["collectionPrice"]) * 1000000000000000000;
    web3.eth
      .sendTransaction({
        from: account,
        to: collectionData["collectionOwner"],
        value: value,
      })
      .on("confirmation", () => {
