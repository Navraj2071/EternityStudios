...
const Deploynft = (props) => {
  ...
  const deployButton = () => {
    mycontract
      .deploy({
        data: bytecode,
        arguments: [props.tokenName, props.tokenSymbol, props.tokenURI],
      })
      .send({ from: props.account }, (err, transactionHash) => {
  ...
  return (
    <>
      <button className="btn" onClick={deployButton}>
        Deploy NFT
      </button>
      <h4 style={{ color: "green" }}>{nftStatus1}</h4>
      <h4 style={{ color: "green" }}>{nftStatus2}</h4>
      <h4 style={{ color: "green" }}>{nftStatus3}</h4>
    </>
  );
};
